...
export default function DoctorsList() {
  ...
  }, [currentPage, selectedSpecialty, gender, experience, searchQuery]);

  const paginate = (pageNumber: number) => setCurrentPage(pageNumber);
  const totalPages = Math.ceil(totalDoctors / doctorsPerPage);

  // Remove client-side filtering since we're doing it on the backend
  const displayedDoctors = doctorData;
  ...
  <p className="text-sm text-gray-500">Minimum years of experience: {experience}+</p>
</div>
        ...
        <div className="md:col-span-3">
          ...
          <div className="relative w-full max-w-lg mx-auto mb-4">
            <input
              type="text"
              value={searchQuery}
              placeholder="Search doctors by name, city, street, or state"
